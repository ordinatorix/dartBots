...
final _contractAbi = _i1.ContractAbi.fromJson(
    ...
    'Chainlink_token_usd_price_aggregator');

class Chainlink_token_usd_price_aggregator extends _i1.GeneratedContract {
  Chainlink_token_usd_price_aggregator(
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> setValidatorConfig(
      _i1.EthereumAddress _newValidator, BigInt _newGasLimit,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[36];
    assert(checkSignature(function, 'eb457163'));
    final params = [_newValidator, _newGasLimit];
    return write(credentials, transaction, function, params);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<ValidatorConfig> validatorConfig({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[42];
    assert(checkSignature(function, '8e0566de'));
    final params = [];
    final response = await read(function, params, atBlock);
    return ValidatorConfig(response);
  }
  ...
  /// Returns a live stream of all ValidatorConfigSet events emitted by this contract.
  Stream<ValidatorConfigSet> validatorConfigSetEvents(
      {_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('ValidatorConfigSet');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
      return ValidatorConfigSet(decoded);
    });
  }
}
...
class ValidatorConfig {
  ValidatorConfig(List<dynamic> response)
      : validator = (response[0] as _i1.EthereumAddress),
        gasLimit = (response[1] as BigInt);
...
class ValidatorConfigSet {
  ValidatorConfigSet(List<dynamic> response)
      : previousValidator = (response[0] as _i1.EthereumAddress),
        previousGasLimit = (response[1] as BigInt),
        currentValidator = (response[2] as _i1.EthereumAddress),
        currentGasLimit = (response[3] as BigInt);

  final _i1.EthereumAddress previousValidator;

  final BigInt previousGasLimit;

  final _i1.EthereumAddress currentValidator;

  final BigInt currentGasLimit;
}
